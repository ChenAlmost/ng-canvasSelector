...
export class AppComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if(changes['defaultArea'].currentValue) {
      this.areaArray = [this.areaArray, ...changes['defaultArea'].currentValue];
    }
  }

  ngAfterViewInit() {
    this.context = this.customCanvas.nativeElement.getContext('2d');
    this.addNewArea();
  }
  ...
  onmouseMove(event: MouseEvent) {
    if (this.isDragging) {
      ...
      const minX = radius;
      const maxX = this.customCanvas.nativeElement.width - radius;
      const minY = radius;
      const maxY = this.customCanvas.nativeElement.height - radius;
